...
Only Docker Hub registry images auto-resolve. A `docker://` image on any other
registry (`ghcr.io`, a private/internal registry, or one that can't be reached)
is **left on its tag** and the build continues with a `pin-unresolvable-registry`
warning rather than hard-failing — so a private registry never becomes an exit-1
cliff, while you're still told what wasn't locked down. The exception is
`--offline`: if the lock can't satisfy a ref it stays **fail-closed** (exit `2`),
because offline determinism is the whole point of the flag.
