...
Actio parses your source with the YAML **1.2** core schema, but the GitHub Actions
runner re-reads the generated workflow with a **1.1**-style schema that silently
coerces bare scalars — `no`→`false`, `1:30`→`90`, `2024-01-01`→a date. A string you
wrote stays a string in Actio yet flips type on the runner. The guard force-quotes
...
(the Norway booleans, binary `0b…`, sexagesimal `1:30`, and ISO timestamps).
