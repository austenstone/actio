...
explicit `name:` whenever you need a stable, human-meaningful artifact name, or when a runtime
`matrix` fans the same step out across legs (the compiler cannot see runtime legs, so it cannot
disambiguate them for you).
