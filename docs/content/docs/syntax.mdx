...
| [`job_defaults`](#job_defaults) | workflow | map of job keys | stripped; merged into every job |
| [`executors`](#executors) | workflow | map of presets | stripped; referenced via `executor` |
| [`injectionHoist`](#injectionhoist) | workflow, job, step | enum | stripped; controls `${{ }}` hoisting in `run:` |
...
| [`dynamic_matrix`](#dynamic_matrix) | job | map | setup job + matrix wiring |
| [`static_if`](#static_if) | job, step | string (bare expr) | node kept or omitted at compile time |
| [`static_if(<expr>)`](#static_if-merge) | job, step | keyed map | merged into parent when true |
| [`inject`](#inject) | step | string | replaced by the fragment's steps |
| [`for_each`](#for_each) | step | map | expanded into repeated steps (or a matrix) |
...
| [`on_success`](#on_success) | job, step | list of steps | guarded steps (`success()`) |
| [`on_failure`](#on_failure) | job, step | list of steps | guarded steps (`failure()` / outcome) |
| [`on_abort`](#on_abort) | job, step | list of steps | guarded steps (`cancelled()`) |
...
<TypeTable
  type={{
    ...
    permissions: { type: 'map', description: 'Replaces on presence; inline wins.' },
    concurrency: { type: 'map', description: 'Replaces on presence; inline wins.' },
    strategy: { type: 'map', description: 'Replaces on presence; inline wins.' },
  }}
/>

Precedence is **inline > extends > job_defaults**: the template runs before `job_defaults`, so
a job written as just `{ extends, with }` already carries a real `uses` before `job_defaults`
partitions jobs — and any key the template (or inline job) supplied is already present, so
`job_defaults` won't overwrite it.
...
- **Valid in:** top level
- **Type:** map of outcome branch groups (`on_success`/`on_failure`/`on_abort`), each
...
<TypeTable
  type={{
    on_success: { type: 'job map', description: 'Teardown jobs that run only when the workflow succeeded.' },
    on_failure: { type: 'job map', description: 'Teardown jobs that run only when the workflow failed.' },
    on_abort: { type: 'job map', description: 'Teardown jobs that run only when the workflow was cancelled.' },
  }}
/>
...
Name (or ordered list of names) from top-level [`call_templates:`](#call_templates). Materializes
...
See [`call_templates`](#call_templates) for the merge rules.

{/* macro:dynamic_matrix */}
### `jobs.<job_id>.dynamic_matrix` [#dynamic_matrix]
...
{/* macro:static_if */}
### `jobs.<job_id>.static_if` [#static_if]
...
Full guide: [static_if](/docs/macros/static-if).

{/* macro:static_if() */}
### `static_if(<expr>)` [#static_if-merge]

Conditional merge form. A key shaped like `static_if(<expr>)` maps to an object that is
...
{/* macro:for_each */}
### `jobs.<job_id>.steps[*].for_each` [#for_each]
...
- **Valid in:** `jobs.<job_id>.steps[*]`
- **Type:** map
- **Compiles to:** repeated steps (serial) or matrix wiring (`parallel: true`)
...
<Callout type="warn" title="Not in the JSON Schema">
  `for_each` is a shipping macro but is **not** described in `actio.schema.json` yet, so
  the editor extension will not autocomplete or validate it. Mistyped options fail at
  `actio build`, not in your editor.
</Callout>
...
<TypeTable
  type={{
    ...
    retry: { type: 'map', description: 'Re-run the guarded step in a sibling job on a different runner when it fails. Keys: runs-on (required), when_exit_code (run-only, POSIX shell only).' },
  }}
/>
...
{/* macro:if_changed */}
### `jobs.<job_id>.steps[*].if_changed` [#if_changed]
...
See [`injectionHoist`](#injectionhoist). Full guide: [injection-hoist](/docs/macros/injection-hoist).
...
`ensure`, `on_success`, `on_failure`, and `on_abort` attach teardown/outcome steps to a
...
| `on_success` | `success() && steps.<id>.outcome == 'success'` | `if: success()` |
| `on_failure` | `!cancelled() && steps.<id>.outcome == 'failure'` | `if: failure()` |
| `on_abort` | step-level cancellation | `if: cancelled()` |
