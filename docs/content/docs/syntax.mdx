...
| [`_anchors`](#_anchors) | workflow | anchor library | stripped; `- *alias` step lists flattened in place |
| [`templates`](#templates) | workflow | map of param'd step lists | stripped; `inject ... with` expanded in place |
| [`fragments`](#fragments) | workflow | map of step lists | **deprecated**; prefer `_anchors:`/`templates:`. stripped; `inject` expanded in place |
...
| [`expand_matrix`](#expand-matrix) | job | boolean | literal matrix unrolled into named jobs |
...
| [`inject`](#inject) | step, job | string | replaced by the fragment's steps, or an imported step/job |
...
| [`soft_fail`](#soft_fail) | step | boolean or list | `continue-on-error`, or a build-time exit-code wrapper |
| [`artifacts`](#artifacts) | step | map | trailing `actions/upload-artifact` step |
...
call site. Full guide: [fragments + inject](/docs/macros/fragments).
...
See [`fragments`](#fragments) and [`templates`](#templates). Full guide: [fragments + inject](/docs/macros/fragments).
...
Expose a step's outputs by a rename-safe logical handle. Reference anywhere with
`${{ ref.<handle>.<output> }}`; Actio assigns the step id, synthesizes
`jobs.<id>.outputs`, adds `needs` edges, and strips the `ref` key. Where `share`
wires compiler-minted outputs, `ref` generalizes the same engine to outputs the
compiler did not mint: action `uses:` outputs, hand-written `$GITHUB_OUTPUT` writes,
and reusable call-job outputs.

- **Valid in:** `jobs.<job_id>.steps[*]`
- **Type:** list of output names `[a, b]` (positional shorthand, primary), or a map with optional `handle` and a required `outputs` list
- **Compiles to:** `steps.<id>.outputs.<name>` (same job) or `needs.<job>.outputs.<name>` (cross job), plus synthesized `job.outputs` and `needs`

Non-minted producers must declare their `outputs:` (auto-sniffing of
`$GITHUB_OUTPUT` is deferred). The positional `ref: [a, b]` is the primary form and
derives the handle from the step `name`, then its id; the `{ handle, outputs }` map
form is the escape hatch for renaming the handle. Cross-job references resolve to
`needs.<job>.outputs.<name>`; same-job references stay local as
`steps.<id>.outputs.<name>` with no `needs` added. A dotted
`${{ ref.<handle>.<output>.<field> }}` lowers to `fromJSON(...)`. Reusable
call-jobs are referenced as `${{ ref.job.<call_id>.<output> }}` and add only a `needs`
edge.
...
<TypeTable
  type={{
    ...
    outputs: { type: 'string[]', description: 'Output names this step publishes. Required for non-minted producers (action, hand-written run). The value of the positional shorthand ref: [..].', required: true },
  }}
/>
