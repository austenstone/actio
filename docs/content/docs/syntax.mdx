...
core schema, so tokens like `no`, `on`, `1:30`, and `2024-01-01` survive as plain
strings. GitHub Actions re-reads the generated workflow with a YAML 1.1-ish schema that
silently coerces them (`no` → `false`, `1:30` → `90`, `2024-01-01` → a date). In `fix`
